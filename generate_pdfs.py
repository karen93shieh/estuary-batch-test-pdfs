...
Writes <out_dir>/pdfs/NNN-slug.pdf, <out_dir>/manifest.json, <out_dir>/urls.txt,
<out_dir>/batch_request.json (all 100 items) and <out_dir>/batch_request_10.json.
Deterministic: the same seed always reproduces the same documents.
Every character carries unique, verifiable facts (passphrase, lucky number,
favourite dish, pet) so knowledge-bank retrieval can be checked after ingestion.
...
assert len(FIRST_NAMES) == COUNT and len(set(FIRST_NAMES)) == COUNT
...
PET_NAMES = ["Biscuit", "Admiral", "Pockets", "Marigold", "Thimble", "Grumble", "Sprocket", "Dumpling", "Ledger",
             ...
             "Marzipan", "Quibble", "Radish", "Sable", "Tinker", "Umbra", "Velvet", "Whistle", "Yolk", "Ziggy", "Amos"]
...
GOALS = ["to reopen the shop their parents lost", "to map every path through {loc}", "to find out what happened to the missing caravan",
         "to be trusted by {faction} again", "to retire somewhere warm with {pet}", "to win the harvest fair for the fifth time",
         "to teach one apprentice who actually listens", "to pay off the last of the debt by winter", "to see the ocean once",
         "to prove {rival} wrong about the old tunnels", "to keep {friend} out of trouble for one full season",
...
FEARS = ["fire in enclosed spaces", "being forgotten", "the bells at midnight", "letting {friend} down", "open water",
...
EVENT_TEMPLATES = [
    ...
    "A dispute over {topic} flared up between {name} and {rival} outside {loc}; it ended, as usual, with neither of them conceding.",
    ...
    "The weather turned. {name} moved the {thing} indoors, fed {pet}, and wrote three lines to {mentor} about {topic}.",
    "{name} was hired to carry a {thing} from {loc} to {loc2} without opening it, and mostly succeeded.",
    "At {loc}, {name} taught {friend} how to tell a real {thing} from a fake one by weight alone.",
    "Rumours about {topic} reached {loc}. {name} pretended not to care and then asked everyone about it.",
    "{name} won {n} {currency} from {rival} on a bet about {topic} and spent all of it on {dish}.",
    ...
    "{mentor} visited unannounced. They argued about {topic} for an hour and parted better friends than before.",
    "{name} closed early, walked to {loc2}, and watched the lights come on. {pet} followed at a distance.",
    "Someone left a {thing} on the doorstep with no note. {name} suspects {friend} and is probably right.",
]
...
CJK_ZH = "{name}是{setting_en}的一名{role_en}。{name}的幸运数字是{lucky}。{name}最喜欢的菜是{dish}。{name}养了一只叫{pet_name}的{species_en}。进入工坊的口令是「{password}」。"
CJK_JA = "{name}は{setting_en}の{role_en}です。{name}のラッキーナンバーは{lucky}です。好きな料理は{dish}です。{pet_name}という名前の{species_en}を飼っています。工房の合言葉は「{password}」です。"
...
def make_characters(rng: random.Random):
    surnames = [rng.choice(SURNAMES) for _ in range(COUNT)]
    dishes = rng.sample([f"{p} {d}" for p in DISH_PREPS for d in DISHES], COUNT)
    pets = rng.sample([(n, s) for n in PET_NAMES for s in PET_SPECIES], COUNT)
    lucky = rng.sample(range(100, 1000), COUNT)
    passwords = rng.sample([f"{a}-{b}" for a in PASS_WORDS for b in PASS_WORDS if a != b], COUNT)
    chars = []
    ...
    for i, ch in enumerate(chars):
        ch["mentor"] = chars[(i - 7) % COUNT]["name"]
        ch["rival"] = chars[(i + 13) % COUNT]["name"]
        ch["friend"] = chars[(i + 41) % COUNT]["name"]
    for ch in chars:
        s = ch["_setting"]
        fmt = dict(
            name=ch["first_name"], rival=ch["rival"], friend=ch["friend"], mentor=ch["mentor"], pet=ch["pet_name"],
...
def page_targets(rng: random.Random):
    targets = ([rng.randint(1, 3) for _ in range(55)] + [rng.randint(4, 8) for _ in range(30)]
               + [rng.randint(10, 25) for _ in range(12)] + [rng.randint(40, 60) for _ in range(3)])
    rng.shuffle(targets)
    return targets


def chronicle(ch, n_entries, rng: random.Random):
    s = ch["_setting"]
    out = []
    ...
    for k in range(n_entries):
        ...
        for _ in range(rng.randint(3, 5)):
            ...
            sentences.append(t.format(
                name=ch["first_name"], rival=ch["rival"].split()[0], friend=ch["friend"].split()[0],
                mentor=ch["mentor"].split()[0], other=rng.choice(FIRST_NAMES), pet=ch["pet_name"],
                loc=locs[0], loc2=locs[1], faction=s["faction"], currency=s["currency"], n=rng.randint(2, 90),
                thing=rng.choice(THINGS), topic=rng.choice(TOPICS), dish=ch["favorite_dish"],
            ))
        out.append((f"Entry {k + 1}, {rng.choice(SEASONS)} of year {year}", " ".join(sentences)))
    return out


def build_pdf(path, ch, target_pages, cjk, rng: random.Random, styles):
    s = ch["_setting"]
    doc = SimpleDocTemplate(
        path, pagesize=LETTER, leftMargin=inch, rightMargin=inch, topMargin=inch, bottomMargin=inch,
        title=f"{ch['name']}: Character Dossier", author="Estuary synthetic test fixtures",
    ...
    first = ch["first_name"]
    story = [
        Paragraph(esc(ch["name"]), TITLE),
        Paragraph("Character Dossier", H2),
        Spacer(1, 6),
        P(ch["tagline"], SMALL),
        Spacer(1, 18),
        ...
        Spacer(1, 6),
        P(f"{first} learned the trade from {ch['mentor']}, who still drops by unannounced. "
          f"The longest-running feud in {first}'s life is with {ch['rival']}, over a matter neither will explain. "
          f"The person {first} trusts most is {ch['friend']}."),
        Spacer(1, 6),
    ...
    facts = [
        ["Full name", ch["name"]], ["Role", ch["role"]], ["Home", ch["setting"]], ["Hometown", ch["hometown"]],
        ["Age", str(ch["age"])], ["Favourite dish", ch["favorite_dish"]],
        ["Pet", f"{ch['pet_name']}, a {ch['pet_species']}"], ["Lucky number", str(ch["lucky_number"])],
        ["Workshop password", ch["password"]], ["Mentor", ch["mentor"]], ["Rival", ch["rival"]], ["Closest friend", ch["friend"]],
    ]
    tbl = Table([[Paragraph(esc(a), SMALL), Paragraph(esc(b), BODY)] for a, b in facts], colWidths=[1.6 * inch, 4.6 * inch])
    ...
    story += [
        tbl, Spacer(1, 10), Paragraph("Secret", H1), P(ch["secret"]), Spacer(1, 10),
        ...
        P(f"{first}: {cap(ch['favorite_dish'])}. Do not tell {ch['rival'].split()[0]} I said so."),
    ...
    if cjk:
        ...
        story += [Spacer(1, 10), Paragraph("Summary (Chinese)", H1), Paragraph(esc(CJK_ZH.format(**fmt)), styles["zh"]),
                  Spacer(1, 6), Paragraph("Summary (Japanese)", H1), Paragraph(esc(CJK_JA.format(**fmt)), styles["ja"])]
    n_entries = int(max(0, target_pages - 1.6) * 8.5)
    if n_entries:
        ...
        for head, body in chronicle(ch, n_entries, rng):
...
def verify(path, ch):
    ...
    ok = all(str(v) in text for v in (ch["name"], ch["password"], ch["lucky_number"], ch["favorite_dish"], ch["pet_name"]))
    return {"pages": pages, "facts_extractable": ok, "chars": len(text)}
...
def main():
    ...
    chars = make_characters(rng)
    targets = page_targets(rng)
    styles = make_styles()
    manifest, items = [], []
    for ch, target in zip(chars, targets):
        fname = f"{ch['index']:03d}-{slugify(ch['name'])}.pdf"
        path = os.path.join(pdf_dir, fname)
        cjk = ch["index"] % 20 == 7
        build_pdf(path, ch, target, cjk, rng, styles)
        url = f"{base_url}/{fname}"
        check = verify(path, ch) or {}
        first = ch["first_name"]
        entry = {
            "index": ch["index"], "file": f"pdfs/{fname}", "url": url, "size_bytes": os.path.getsize(path),
            "pages": check.get("pages"), "facts_extractable": check.get("facts_extractable"),
            "has_cjk_section": cjk, "target_pages": target,
            "character": {k: ch[k] for k in ("name", "role", "setting", "tagline", "traits", "speech", "age", "hometown",
                                             "favorite_dish", "pet_name", "pet_species", "lucky_number", "password",
                                             "mentor", "rival", "friend", "goal", "fear", "secret")},
            "retrieval_checks": [
                {"question": f"What is {first}'s favourite dish?", "expect": ch["favorite_dish"]},
                {"question": f"What is {first}'s lucky number?", "expect": str(ch["lucky_number"])},
                {"question": f"What is the password to {first}'s workshop?", "expect": ch["password"]},
                {"question": f"What is the name of {first}'s pet?", "expect": ch["pet_name"]},
                {"question": f"Who is {first}'s rival?", "expect": ch["rival"]},
            ],
        }
        manifest.append(entry)
        items.append({
            "customId": f"test-npc-{ch['index']:03d}-{slugify(first)}",
            "persona": {
                "name": ch["name"],
                "tagline": ch["tagline"],
                "personality": f"{cap(ch['traits'])}. {first} {ch['speech']}.",
                "background": f"{cap(ch['role'])} of {ch['setting']}, originally from {ch['hometown']}. "
                              f"Detailed lore, including verified facts, is in the attached dossier.",
            },
            "documents": [{"url": url, "name": f"{ch['name']} dossier", "type": "pdf"}],
        })
        print(f"{fname:48s} target={target:2d} pages={check.get('pages')} ok={check.get('facts_extractable')} "
              f"{entry['size_bytes']:>7d}B")
    with open(os.path.join(out_dir, "manifest.json"), "w", encoding="utf-8") as f:
        json.dump({"seed": SEED, "base_url": base_url, "count": len(manifest), "documents": manifest}, f, indent=2,
                  ensure_ascii=False)
    with open(os.path.join(out_dir, "urls.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(e["url"] for e in manifest) + "\n")
    ...
    total = sum(e["size_bytes"] for e in manifest)
    pages = sum(e["pages"] or 0 for e in manifest)
    bad = [e["file"] for e in manifest if e["facts_extractable"] is False]
    print(f"\n{len(manifest)} PDFs, {total / 1e6:.1f} MB, {pages} pages total; extraction failures: {bad or 'none'}")
